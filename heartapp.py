...
heart = pd.read_csv("C:\\Users\GOPI KRISHNA\Desktop\heart.csv")
...
def takeInput():
    ...
    inputValues.append(int(sex.get()))  # Convert sex to integer
    inputValues.append(int(chestPain.get()))  # Convert chestPain to integer
    inputValues.append(trestbps1)
    inputValues.append(chol1)
    inputValues.append(int(FBS.get()))  # Convert FBS to integer
    inputValues.append(int(ECG.get()))  # Convert ECG to integer
    inputValues.append(thalach1)
    inputValues.append(trestbps1)  # This line seems redundant, you might want to remove it
    inputValues.append(oldpeak1)
    inputValues.append(int(slope.get()))  # Convert slope to integer
    inputValues.append(int(ca.get()))  # Convert ca to integer
    inputValues.append(int(thal.get()))  # Convert thal to integer

    input_df = pd.DataFrame([inputValues], columns=X.columns)  # Create a DataFrame with appropriate column names

    final_Result = knn_classifier.predict(input_df)
